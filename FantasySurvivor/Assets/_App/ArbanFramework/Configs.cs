			...
			if(reader == null)
			{
				try
				{
					...
					if(string.IsNullOrEmpty(text))
					{
						...
						string fullPath = "";
#if UNITY_EDITOR
						fullPath = "Configs/" + FileName;
						textAsset = Resources.Load<TextAsset>(fullPath);
#else
						fullPath = Path.Combine(Application.streamingAssetsPath, $"{FileName}.txt");
						if(File.Exists(fullPath))
						{
							textAsset = new TextAsset(File.ReadAllText(fullPath));
						}
						else
						{
							fullPath = "Configs/" + FileName;
